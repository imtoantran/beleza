...
SQL;
	...
	/**
	 * Tìm mã gift voucher
	 * @param $_POST['e_voucher_id'] : mã của gift voucher đó
	 * @return json
	 */
	public function get_redeem_gvoucher() {
		$user_id = Session::get('user_id');
		$g_voucher_code = $_POST['g_voucher_code'];

		$aQuery = <<<SQL
		SELECT 
			g.gift_voucher_id,
			g.gift_voucher_code,
			g.gift_voucher_due_date, 
			g.gift_voucher_date, 
			g.gift_voucher_status,
			g.gift_voucher_price,
			g.gift_voucher_email
		FROM 
			gift_voucher g
		WHERE 
			g.gift_voucher_code = '{$g_voucher_code}'
SQL;
		$data = $this->db->select($aQuery);
	...
	/**
	 * Xác nhận dùng gift voucher này
	 * @param $_POST['data_id'] : mã của gvoucher đó
	 * @return success/errorr
	 */
	public function update_gift_voucher() {
		$user_id = Session::get('user_id');
		$data_id = $_POST['data_id'];
		$data = array(
			"gift_voucher_status" => 1,
			"gift_voucher_user_id" => $user_id
		);

		if( $this->db->update('gift_voucher', $data, "gift_voucher_id = '$data_id'") ){
			echo 'success';
		} else {
			echo 'error';
		}
...
SQL;
	...
	/**
	 * Danh sách những lịch hẹn sắp tới chưa được xác nhận 
	 * @param $user_id
	 * @return success/error
	 */
