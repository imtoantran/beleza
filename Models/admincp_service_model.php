 ...
 *
 */
class admincp_service_model extends Model {
...
SQL;
		...
		if ($select) {
			...
			foreach ($select as $row) {
				$sOutput .= "[";
				$sOutput .= '"' . str_replace('"', '\"', $row['service_id']) . '",';
	...
	public function saveService($data) {
		...
		if(!$service_image["error"]) {
			$file_name = str_replace(" ", "", $service_image["name"]);

			if($file_name != '') {
				$tmp_name = $service_image["tmp_name"];
...
SQL;
	...
	public function loadServiceDetailEdit($service_id) {
		$sql = <<<SQL
SELECT service_id
, service_name
, service_image
	...
	public function editService($data) {
		...
		if(!$service_image["error"]) {
			...
			if($file_name != '') {
			    ...
			    $update_image = ", service_image = '{$path_insertSQL}'";
			}
		} else {
			$path_insertSQL = '';
			$update_image = '';
...
, service_service_type_id = {$data['service_type_id']}
