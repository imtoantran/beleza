...
/* imtoantran start bookmark */
var bookmark = function bookmark(e) {
    var _ = this;
    var loading = $(_).find("span.loading");
    var _class = loading.attr("class");
    loading.attr("class","fa fa-refresh fa-spin");
    ...
    $.ajax({
       url: URL + $(_).data("controller"),
        ...
        success:function(response){
            if(response.success){
                $(_).removeClass("notbookmark");
                $(_).find(".text").text(response.message);
            }
        },
        complete:function(){
            loading.attr("class",_class);
            $(_).attr("disabled",false);
        }
    });
};
/* imtoantran stop bookmark */
$(document).ready(function () {
    ...
    $("#success_review").click(function () {
        $(this).fadeOut();
    });

    $("body").on("click","#bookmark .notbookmark",bookmark);
});
            ...
            if (response.user[0] != null) {
                $.each(response.user[0], function (key, value) {
                    ...
                    if (key == 'user_description') {
                        ...
                        /* imtoantran add excerpt to user description start */
                        im_id = "#user_location_description";
                        if ($(im_id).height() > 200) {
                            txtHide = "Ẩn đi";
                            txtShow = "Xem thêm";
                            button = '<div style="margin-top: 8px;margin-bottom: 8px;margin-right:5px;" class="text-right"><button class="btn btn-brown btn-xs im-see-more"><small><span class="im-see-more-text">' + txtShow + '</span></small></button></div>';
                            $(im_id).after(button);
                            $(im_id).css({
                                height: '200px',
                                overflow: 'hidden',
                            ...
                            $(".im-see-more").click(function () {
                                im_showMore(im_id, ".im-see-more-text", txtHide, txtShow)
                            });
                        }
                        /* imtoantran add excerpt to user description end */
                    }
                    if (key == 'user_slide') {
                    ...
                    if (key == 'user_open_hour') {
                        ...
                        var gift_voucher = response.array_voucher.gift_voucher;

                        var num_evoucher = response.array_voucher.num_evoucher;
                        var num_appointment = response.array_voucher.num_appointment;
                        var num_evoucher_appointment = response.array_voucher.num_evoucher_appointment;

                        $('#location_open_hour').html(user_open_hour);

                        if (num_evoucher_appointment <= 0) {
                            if (num_evoucher <= 0) {
                                $('#use_e_voucher').show();
                            }
...
function loadReview() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.data[0] != null) {
                $.each(response.data, function (key, value) {
                    ...
                    html += '<h5 class="media-heading"><strong>' + value.client_username + '</strong><span class="rating">' + value.user_review_overall + '</span><small class="pull-right"><i>tham gia ' + formatDate(client_join_date) + '&nbsp</i></small></h5>';
        ...
        complete: function () {
            ...
            $("#review_field").delegate('.reply-input', 'keypress', function (e) {
                checkContent(e);
            });
            $("#review_field").delegate(".collapse", "show.bs.collapse", function (e) {
                // body...
                id = $(this).attr("id").replace("reply-colls-");
                $(".show-reply-input button").hide();
                // $(".show-reply-input button").text("Ẩn đi");
            });
            $("#review_field").delegate(".collapse", "hide.bs.collapse", function (e) {
                // body...
                id = $(this).attr("id").replace("reply-colls-");
    ...
    if (USERNAME != '') {
        $.ajax({
            ...
            success: function (response) {
                var html = '';
                var html_2 = '';
                rating['user_review_active'] = response.user_review[0]['user_review_active'];
                rating['user_review_clean'] = response.user_review[0]['user_review_clean'];
                rating['user_review_quality'] = response.user_review[0]['user_review_quality'];
                rating['user_review_staff'] = response.user_review[0]['user_review_staff'];
                rating['user_review_valuable'] = response.user_review[0]['user_review_valuable'];
                if (0 < response.user_review[0]['user_review_overall'])
                    isRated = true;
                ...
                if (response.user_review[0] != null) {
                    $.each(response.review_type, function (key, value) {
                        ...
                        className = value.review_id == "1" ? "" : "act";
                        for (var i = 1; i <= star; i++) {
                            html += '<span data-field="' + value.review_field + '" data-rating="' + i + '" class="fa fa-star choosen rating_text ' + className + '"></span>';
                        }
                        for (var j = star + 1; j <= 5; j++) {
                            html += '<span data-field="' + value.review_field + '" data-rating="' + j + '" class="fa fa-star-o rating_text ' + className + '"></span>';
                        }
                        html += '</span>';
                        html += '</div>';
                        html += '</div>';
            ...
            complete: function () {
                $('.fa.rating_text.act').on('mouseover', function () {
                    ...
                    $('.fa.rating_text.act').each(function () {
...
function sendRating(field, rating_value) {
    isRated = true;
    rating[field] = rating_value;
    var overall = 0;
    $.each(rating, function (i, e) {
        overall += parseInt(e);
    })

    overall = Math.floor(overall / 5);

    $("[data-field='user_review_overall']").each(function (i, v) {
        if (i < overall) {
...
function sendReview() {
    if (($('#review_form').val()).length < 10) {
        ...
        if (!isRated) {
            location.href = "#review_input";
            return alert("Hãy bình chọn sao cho địa điểm này");
        }
