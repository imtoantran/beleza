...
/*LOAD CLIENT HOBBY*/
function loadClientHobby() {
	$.ajax({
		url : URL + 'service/loadClientHobby',
		type : 'post',
		dataType : 'json',
		data : {
			client_id : CLIENT_ID,
		...
		success : function(response) {
			if (response[0] != null) {
				HAS_HOBBY = 1;
				CLIENT_HOBBY_ANSWER = response[0].hobby_answer.split(',');
				CLIENT_HOBBY_RULE = response[0].hobby_rule;
				console.log(CLIENT_HOBBY_ANSWER);
				cfirm = confirm('Bạn đã trả lời rồi, bạn có muốn trả lời lại không?');
				if (cfirm == false) {
					IS_SAVE = 0;
					$.ajax({
						url : URL + 'service/loadClientHobbyRule',
						type : 'post',
						dataType : 'json',
						data : {
							client_hobby_rule : CLIENT_HOBBY_RULE,
						},
						success : function(response) {
							RULE_SERVICE_ID = response[0].rule_service_id;
							HOBBY_ANSWER = response[0].rule_group;
...
function loadConsultingQuestion() {
	$.ajax({
		...
		complete : function() {
			if (HAS_HOBBY == 1) {
				$('input[name="consult_answer"]:visible').each(function() {
					for (var i = 0; i < CLIENT_HOBBY_ANSWER.length; i++) {
						if (CLIENT_HOBBY_ANSWER[i] == $(this).val()) {
