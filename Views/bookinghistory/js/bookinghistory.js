$(document).ready(function() {
	loadClientBookingList();
	xhrGet_appointment_time_end_for_edit();
	xhrUpdate_booking_detail();
});

var loadClientBookingList = function() {
	var btnOM_cb = $("#booking_history_table tbody").find("tr");
	var cb_table = $("#booking_history_table");
	var cb_modal = $('#confirm_booking_modal');
	var cb_form = $('#confirm_booking_form');

	var html = '<tr data_id=":booking_detail_id" class=":alert" :requestChange>'
			+	'<td>:booking_id</td>'
			+	'<td>:booking_detail_id</td>'
			+	'<td>:user_business_name</td>'
			+	'<td>:user_service_name</td>'
			+	'<td>:booking_detail_price</td>'
			+	'<td>:booking_detail_date - :booking_detail_time_start</td>'
			+	'<td>:booking_date</td>'
	...
	var status_0 = '<span class="label label-primary">Chưa Xong</span>';
	var status_1 = '<span class="label label-success">Hoàn Thành</span>';
	var status_2 = '<span class="label label-danger">Đã Hủy</span>';

	var status_isConfirm_0 = '<span class="label label-warning">Đang Chờ</span>';
	var status_isConfirm_1 = '<span class="label label-success">Chấp Nhận</span>';

	var url = URL + 'bookinghistory/loadClientBookingList';
	$.get(url, function(data) {
        if(data.length > 0) {
        	var current_date = moment().format("YYYY-MM-DD");
        	$.each(data, function(index, value) {
        		out = html.replace(/:booking_detail_id/g, value['booking_detail_id']);
        		out = out.replace(':booking_id', value['booking_id']);
        		out = out.replace(':booking_date', moment(value['booking_date'], "YYYY-MM-DD").format("DD/MM/YYYY"));
        		out = out.replace(':user_business_name', value['user_business_name']);
        		out = out.replace(':user_service_name', value['user_service_name']);
        		out = out.replace(':booking_detail_price', $.number(value['booking_detail_price']));
        		out = out.replace(':booking_detail_date', moment(value['booking_detail_date'], "YYYY-MM-DD").format("DD/MM/YYYY") );
        		out = out.replace(':booking_detail_time_start', value['booking_detail_time_start'].substr(0,5));
        		out = out.replace(':booking_detail_is_confirm', value['booking_detail_is_confirm'] == 1 ? status_isConfirm_1 : status_isConfirm_0);
        		out = out.replace(':booking_detail_status', value['booking_detail_status'] == 1 ? status_1 : value['booking_detail_status'] == 2 ? status_2 : status_0);
        		out = out.replace(':alert', value['booking_detail_request_change'] == 1 ? 'alert-danger' :  value['booking_detail_date'] == current_date ? 'alert-success' : '');
        		out = out.replace(':requestChange', value['booking_detail_request_change'] == 1 ? 'requestChange="1"' : '');
    ...
    .done(function(){
    	cb_table.dataTable({
			...
			"oLanguage" : {
				"sZeroRecords" : "Không có dữ liệu lịch sử cuộc hẹn nào cả.",
				"sSearch" : "Tìm kiếm: ",
        ...
        var row_request_change = $("#booking_history_table tbody").find("tr[requestchange=1]");
