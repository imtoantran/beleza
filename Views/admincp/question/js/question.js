$(document).ready(function() {
	...
	$('.question_answer').keydown(function(event) {
	...
	$('#question_is_usual').on('change', function() {
		if ($('#question_is_usual').is(':checked')) {
			QUESTION_IS_USUAL = 1;
		} else {
			QUESTION_IS_USUAL = 0;
		}
							...
							$('div.done').fadeIn(function() {
								$.ajax({
									...
									data : {
										...
										question_is_usual : QUESTION_IS_USUAL
									},
									success : function(response) {
										if (response == 200) {
...
function loadQuestionDetail() {
	$.ajax({
		...
		success : function(response) {
			if (response[0] != null) {
				$.each(response[0], function(key, value) {
					$('#' + key).val(value);
					if (key == 'question_is_usual') {
						if (value == 1) {
							QUESTION_IS_USUAL = 1;
							$('#question_is_usual').prop('checked', true);
						}
					}
		...
		complete : function() {
			$('.fact_answer').keydown(function(event) {
				var e = event || window.event;
				var keycode = (event.keyCode ? event.keyCode : event.which);
				if (keycode == '220') {
					e.preventDefault();
