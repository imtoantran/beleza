			...
			if (response[0] != null) {
				...
				$.each(response, function(key, value) {
					...
					$.each(value.fact, function(i, item) {
						...
						html += '<b>' + item.fact_answer + '</b> (ID = '+item.fact_id+')</p>';
					});
					index++;
				});
				$('#question_list').html(html);
			} else {
				$('#question_list').html('<div class="question-field"><p class="question">1) ...</p><p fact-id="" question-id="" class="answer pointer"><b>...</b></p><p fact-id="" question-id="" class="answer pointer"><b>...</b></p></div>');
			}
		},
		complete : function() {
