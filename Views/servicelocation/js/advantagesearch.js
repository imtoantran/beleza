$(document).ready(function() {
	...
	$('#city_id_advance').on('change', function() {
		CITY_ID = $(this).val();
		SERVICE_TYPE_SEARCH = '';
		SERVICE_SEARCH = '';
		loadResultSearch(1);
		...
		success : function(response) {
			...
			if (response.service_type[0] != null) {
				...
				$.each(response.service_type, function(key, value) {
					...
					html += '<div>';
					html += '<input id="filter-4-st'+key+3+'" value="' + value.service_type_id + '" type="radio" name="service_type">';
					html += '<label for="filter-4-st'+key+3+'">&nbsp;&nbsp;' + shorten(value.service_type_name, 30) + '</label>';
			...
			if (response.service[0] != null) {
				$.each(response.service, function(key, value) {
					...
					html_2 += '<div>';
					html_2 += '<input id="filter-4-b'+3+key+'" value="' + value.service_id + '" type="checkbox" name="service">';
					html_2 += '<label for="filter-4-b'+3+key+'">&nbsp;&nbsp;' + shorten(value.service_name, 30) + '</label>';
