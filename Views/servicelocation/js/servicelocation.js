$(document).ready(function() {
	if (CITY_ID == '') {
		...
		if(DISTRICT_ID == ''){
			...
			$('#city_id_advance').hide();
		}else{
			$('#city_id_advance_field').hide();
		}	
	}
	if ($('#sort_by').val() == 5) {
		var map;
	...
	$('#waiting_for_result_list').fadeIn(function() {
		$.ajax({
			...
			data : {
				...
				city_id : CITY_ID,
				param_from_m_h : PARAM_FROM_MENU_HOME,
				sort_by : $('#sort_by').val(),
				...
				x_curr : XCURR,
				user_service_sale_price_low: PRICE_SEARCH_LOW,
				y_curr : YCURR
			},
			success : function(response) {
				...
				if (response.data[0] != null) {
					$.each(response.data, function(key, value) {
						$('#city_id_advance option').each(function() {
							if ($(this).val() == value.user_city_id) {
								$(this).show();
							}
						});
						...
						$.each(value.user_service, function(key, item) {
							...
							html += ' <i class="fa fa-lg fa-dollar text-white"></i> <span class="text-white">' + $.number(item.user_service_sale_price) + ' đ</span>';
							html += '</button>';
							html += '</div>';
							html += '</div>';
							html += '</div>';
					...
					if (USER_ADDRESS_1 == '' && USER_ADDRESS_2 == '') {
						//if(INITLOAD == 1){
							//$('#sort_by').find('option').attr('disabled', 'disabled');
							//$('#result-list').html('<h4 class="text-center"><i class="fa fa-gear fa-lg fa-spin"></i> Đang tải từ google...</h4>');
							//loadFromGoogle();
						//}else{
			...
			complete : function() {
				...
				$('.show_map').on('click', function() {
					...
					if (USER_ADDRESS_2_TEMP == '') {
						...
						$('#current_address').text(USER_ADDRESS_2_TEMP + ' (tương đối)');
					}
				});
				...
				$('li.page_number').on('click', function(){
					$('body,html').animate({
						scrollTop : 0
					});
				});	
...
function loadFromGoogle() {
	$.ajax({
		url : URL + 'servicelocation/loadFromGoogle',
		type : 'post',
		data : {
			service_name : SERVICE_NAME,
		},
